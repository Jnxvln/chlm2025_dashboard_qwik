    ...
    <section class="max-w-3xl mx-auto px-4 py-6">
      ...
      <Form action={updateDriver} class="mt-4 flex flex-col gap-4 bg-white border border-gray-200 shadow p-6 rounded-lg">
        <div class="flex gap-4">
          <input name="firstName" type="text" value={driver.value.firstName} required placeholder="First Name"
            class="w-full border border-gray-300 rounded p-2" />
          <input name="lastName" type="text" value={driver.value.lastName} required placeholder="Last Name"
            class="w-full border border-gray-300 rounded p-2" />
        </div>

        <input name="defaultTruck" type="text" value={driver.value.defaultTruck ?? ''} placeholder="Default Truck"
          class="w-full border border-gray-300 rounded p-2" />

        <input name="endDumpPayRate" type="number" step="0.01" value={driver.value.endDumpPayRate}
          placeholder="End Dump Pay Rate"
          class="w-full border border-gray-300 rounded p-2" />

        <input name="flatBedPayRate" type="number" step="0.01" value={driver.value.flatBedPayRate}
          placeholder="Flatbed Pay Rate"
          class="w-full border border-gray-300 rounded p-2" />

        <input name="nonCommissionRate" type="number" step="0.01" value={driver.value.nonCommissionRate}
          placeholder="Non-Commission Rate"
          class="w-full border border-gray-300 rounded p-2" />

        <div class="flex gap-4">
          <input name="dateHired" type="datetime-local"
            value={driver.value.dateHired ? new Date(driver.value.dateHired).toISOString().slice(0, 16) : ''}
            class="w-full border border-gray-300 rounded p-2" />
          <input name="dateReleased" type="datetime-local"
            value={driver.value.dateReleased ? new Date(driver.value.dateReleased).toISOString().slice(0, 16) : ''}
            class="w-full border border-gray-300 rounded p-2" />
        </div>

        <label class="inline-flex items-center gap-2 mt-2">
          <input name="isActive" type="checkbox" value="true" checked={driver.value.isActive}
        ...
        {success.value && (
          <p class="text-green-600 font-medium">Driver updated! Redirecting...</p>
        )}
      </Form>
    </section>
  );
