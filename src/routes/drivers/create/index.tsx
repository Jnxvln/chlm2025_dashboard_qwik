  ...
  return (
    <section>
      ...
      <Form action={createDriverAction} class="mt-4 flex flex-col max-w-xl">
        ...
        <input name="endDumpPayRate" type="number" min={0} step={0.01} placeholder="End Dump Pay Rate *" class="my-2" required />
        <input name="flatBedPayRate" type="number" min={0} step={0.01} placeholder="Flatbed Pay Rate *" class="my-2" required />
        <input name="nonCommissionRate" type="number" min={0} step={0.01} placeholder="Non-commission Rate *" class="my-2" required />

        <div class="flex my-2 items-center gap-2">
          <input name="dateHired" type="datetime-local" placeholder="Date Hired" />
          <input name="dateReleased" type="datetime-local" placeholder="Date Released" />
        </div>

        <div class="flex items-center my-3 mb-4">
          <label for="isActive" class="mr-2 hover:cursor-pointer">Is Active</label>
          <input name="isActive" type="checkbox" id="isActive" value="true" checked class="hover:cursor-pointer" />
        </div>
